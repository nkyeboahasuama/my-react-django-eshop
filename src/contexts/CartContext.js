...
const CartContextProvider = (props) => {
  ...
  useEffect(() => {
    let price = cartItems.reduce((total, item) => total + item.product.price * item.quantity, 0);
  ...
  const handleDecrease = (item) => {
    ...
    if(index !== -1){
      ...
      const newCartItem = [...cartItems, {product: item.product, quantity: 1}]
      setCartItems(newCartItem)
      setCartUpdated(!cartUpdated)
    }
  };
  ...
  const handleRemove = (item) => {
    const newSavedItems = cartItems.filter((cartItem) => cartItem.product.id !== item.product.id);
  ...
  return (
    <CartContext.Provider value={{ cartItems, setCartItems, totalPrice, setTotalPrice, addToCart, onRemove:handleRemove, handleIncrease, handleDecrease, quantity, setQuantity}}>
        {props.children}
